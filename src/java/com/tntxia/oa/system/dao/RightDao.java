...
public class RightDao extends BaseDao{
	 ...
	 * @return
	 */
	public List<String> getRestrainRightList(int restrainId) {

		DBConnection einfodb = new DBConnection();

		String strSQL = "select r.name from [right] r,[role_right_rel] rrr where r.id = rrr.rightId and roleId = ?";

		ResultSet sqlRst = einfodb.executeQuery(strSQL, restrainId);

		List<String> res = new ArrayList<String>();

		try {
			while (sqlRst.next()) {
				res.add(sqlRst.getString("name"));
			}
	...
	public void addRight(Right right) {
		DBConnection einfodb = new DBConnection();
		String sql = SystemCache.sqlmapping.get("addRight");
		List<Object> params = new ArrayList<Object>();
		params.add(right.getRightName());
		params.add(right.getRightDisplay());
		einfodb.executeUpdate(sql, params);
	}

	public void updateRight(Right right) {
		DBConnection einfodb = new DBConnection();
		String sql = SystemCache.sqlmapping.get("updateRight");
		List<Object> params = new ArrayList<Object>();
		params.add(right.getRightName());
		params.add(right.getRightDisplay());
		params.add(right.getId());
		einfodb.executeUpdate(sql, params);
	}
	...
	private void deleteAllRight(int roleId) {
		DBConnection einfodb = new DBConnection();
	 ...
	 * @return
	 */
	public void updateRight(int roleId, String rights) {
		...
		for (String r : rightsSplit) {
			if (r.trim().length() == 0) {
				continue;
			}
			String sql = "insert into role_right_rel(roleId,rightId) values(?,?)";
			List<Object> params = new ArrayList<Object>();
			params.add(roleId);
			params.add(r);
